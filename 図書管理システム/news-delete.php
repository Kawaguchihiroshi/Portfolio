...
<?php
$pdo=new PDO('mysql:host=localhost;dbname=bookmanage;charset=utf8', 'bookmanage', 'kokurihaoasystem');
$sql=$pdo->prepare('update info set unsub=? WHERE id=?');
$sql->execute([
	$_REQUEST['unsub'], 
	$_REQUEST['id']
]);
...
echo '		<div class="col-md-3">投稿日</div>';
echo '	    <div class="col-md-9">', $_REQUEST['do_date'], '</div>';
echo '	</div>';
echo '';
echo '	<div class="row pt-4">';
echo '		<div class="col-md-3">お知らせタイトル</div>';
echo '	    <div class="col-md-9 text-truncate">', $_REQUEST['title'], '</div>';
echo '	</div>';
echo '';
