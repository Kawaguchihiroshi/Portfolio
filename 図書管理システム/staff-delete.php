...
<?php
$pdo=new PDO('mysql:host=localhost;dbname=bookmanage;charset=utf8', 'bookmanage', 'kokurihaoasystem');
$sql=$pdo->prepare('update staff set unsub=? WHERE id=?');
$sql->execute([
	$_REQUEST['unsub'], 
	$_REQUEST['id']
]);

	echo '<h2 class="mt-5">以下の内容を削除いたしました。</h2>';
	echo '<hr>';
	echo '	<div class="row pt-4">';
	echo '		<div class="col-md-3">指導員番号</div>';
	echo '	    	<div class="col-md-5">', $_REQUEST['id'], '</div>';
