...
<title>管理画面 / お知らせ削除｜NERVE FACTORY</title>
</head>
<body>
...
<h1 class="mb-4" style="border-bottom: solid 3px #444;">お知らせ削除</h1>

<?php
$pdo=new PDO('mysql:host=localhost;dbname=shopingcart;charset=utf8', 'shopingcart', 'shopingcart');
$sql=$pdo->prepare('update information set admin_id=?, del=1 where id=?');
$sql->execute([
	$_SESSION['admin']['id'], 
	$_REQUEST['id']
]);
...
echo '<h2 class="mb-4" style="border-bottom: solid 1px #444;">以下、内容を削除しました。</h2>';
echo '<div class="row mb-2">';
echo '<div class="col-md-3 py-2 bg-info text-white rounded">タイトル</div>';

$pdo=new PDO('mysql:host=localhost;dbname=shopingcart;charset=utf8', 'shopingcart', 'shopingcart');
$sql=$pdo->prepare('SELECT * FROM information WHERE id=?');
$sql->execute([$_REQUEST['id']]);
foreach($sql->fetchAll() as $row){
echo '<div class="col-md-9">', $row['title'], '</div>';
echo '</div>';
echo '<div class="row mb-5">';
echo '<div class="col-md-3 py-2 bg-info text-white rounded">本文</div>';
echo '<div class="col-md-9">', $row['content'], '</div>';
}
