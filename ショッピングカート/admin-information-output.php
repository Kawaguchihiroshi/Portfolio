...
$sql=$pdo->prepare('update information set admin_id=?, title=?, content=?, flag=?, status=? where id=?');
$sql->execute([
	$_REQUEST['admin_id'], 
	$_REQUEST['title'], 
	$_REQUEST['content'], 
...
echo '<h2 class="mb-4" style="border-bottom: solid 1px #444;">以下、内容に変更しました。</h2>';
